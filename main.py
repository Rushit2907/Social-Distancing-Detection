...
def get_perspective_transform(H, W):
    inp_point = np.float32(np.array(points[:4]))
    dst_points = np.float32([[0, H], [W, H], [W, 0], [0, 0]])

    # Perspective convolution (3x3)
    M = cv2.getPerspectiveTransform(inp_point, dst_points)

    # Adjusting the original frame according to the perspective convolution M
    perspective_transform = cv2.warpPerspective(frames, M, (H, W))
...
def get_points(event, x, y, flags, params):
    global click, points, mappedDist
    ...
    if event == cv2.EVENT_LBUTTONDOWN and click < 4:
        ...
        if len(points) == 3:
            cv2.line(frame,(x, y),points[-1],(70, 70, 70),2,)
            cv2.line(frame,(x, y),points[0],(70, 70, 70),2,)
    ...
    while True:
        ...
        for id, box in centroid_dict.items():
            if id in red_zone_list:
                cv2.circle(frames,((box[2]+box[4])//2,box[5]),10,(0,0,255),-1)
                red_mat.append(((box[2]+box[4])//2,box[5]))
                cv2.rectangle(frames, (box[2], box[3]), (box[4], box[5]), (0, 0, 255), 2)
            else:
                cv2.circle(frames,((box[2]+box[4])//2,box[5]),10,(0,255,0),-1)
                green_mat.append(((box[2]+box[4])//2,box[5]))
                cv2.rectangle(frames, (box[2], box[3]), (box[4], box[5]), (0, 255, 0), 2)
